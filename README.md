...
Строятся гистограммы для размеров пакетов и пауз между пакетами. Для равномерной модели всё просто - размеры пакетов всегда одинаковые, как и паузы между пакетами, поэтому на гистограмме наблюдаем один столбик. 

![](./images/EQUAL.png)


Для модели с пуассоновским и экспоненциальном распределением также строятся гистограммы. Слева - для пауз между пакетами, справа - для размеров пакетов. График слева имеет пуассоновское распределение, а правый график - экспоненциальное, это значит, что модель на C++ работает верно и генерирует данные с правильными статистическими характеристиками.
![](./images/POISSON.png)
...
Для масштабируемости кода есть интерфейс Imodel, который содержит 1 виртуальный метод. Интерфейс подсказывает, какой минимальный функционал должен реализовывать класс, представляющий модель, чтобы остальной код мог корректно с ним работать.

Классы equal и poisson модели наследуют этот интерфейс и реализуют.
